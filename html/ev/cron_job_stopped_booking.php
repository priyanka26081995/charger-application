...
$transaction_stop_value = 0;
	...
	join transaction_stop as tstop on bboking.transaction_pk  = tstop.transaction_pk 
	where bboking.transaction_pk != 0   
	 ...
	 foreach($chargings as $charging_booking) 
	{
		...
		$charging_booked_amount = $charging_booking['charging_booked_amount'];
			...
			if(!empty($transaction_pk))
			{
				...
				if(!empty($energy_used))
				{
					...
					if($total_amount > $charging_final_amount)
					{
						...
						$charging_final_amount = $total_amount;
					}
	...
	{
		$nru = array();  
		$nru['task_name'] = 'STOPPED_BOOKING_CHECK_CRON_TASK';	 	
		$nru['creation_date'] = CURRENT_DB_DATE;  	
