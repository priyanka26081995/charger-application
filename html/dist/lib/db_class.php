...
public function show_charset(){
	echo "Initial character set is: " .  mysqli_character_set_name($this->link_id);die();
}

public function set_charset($type){
	// echo "Initial character set is: " .  mysqli_character_set_name($this->link_id); 
	mysqli_set_charset( $this->link_id, $type);
...
#-#############################################
# Desc: takes db backup of all tables or tables specified to an open connection
# Param: (table array/string[optional]) to execute 
public function backup_tables($tables = '*'){
	...
	if($tables == '*')
	{
		...
		if (!$query_result){
			$this->oops("<b>MySQL Query fail:</b> SHOW TABLES");
			return 0;
		}
	...
	//save file
	$handle = fopen('db/db-backup-'.time().'.sql','w+');
...
public function fetch($query_id=-false){
	// retrieve row
	 // var_dump($query_id);die();
	 $record = array();
	 
	if (!$query_id){
		if(isset($query_id->num_rows))
		{
...
private function free_result($query_id=false){
	...
	// var_dump(@mysqli_free_result($this->query_id));
...
private function oops($msg=''){
	...
	{ 
		...
		if(!$this->debug) 
		{
			die('MySQL Error:</td><td>'.$this->error);
		...
		else
		{
				...
				<tr><td align="right">Date:</td><td>'.date("l, F j, Y \a\\t g:i:s A").'</td></tr>';
...
$db = Database::obtain(DB_HOST, DB_USERNAME, DB_PASSWORD, DB_NAME); 
 // var_dump($db);die();
 $db->connect();  
